...
def run_ragas_50q(answers: list[dict]) -> list[RagasResult]:
    ...
    for a, pq in zip(answers, per_q):
        results.append(RagasResult(
            ...
            faithfulness=clean_score(pq.faithfulness),
            answer_relevancy=clean_score(pq.answer_relevancy),
            context_precision=clean_score(pq.context_precision),
            context_recall=clean_score(pq.context_recall),
        ))
    return results
...
def cluster_analysis(results: list[RagasResult]) -> dict:
    ...
    # Find dominant failure distribution
    dist_counts = {
        d: sum(matrix[m][d] for m in matrix)
        for d in ["factual", "multi_hop", "adversarial"]
    }
    dominant_dist = max(dist_counts, key=dist_counts.get)

    # Find dominant failure metric
    metric_counts = {
        m: sum(matrix[m].values())
        for m in matrix
    }
