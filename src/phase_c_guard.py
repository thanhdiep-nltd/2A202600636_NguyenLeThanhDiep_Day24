...
async def check_input_rail(text: str, rails=None) -> dict:
    ...
    # Safe string conversion from dict / list / string / object
    response_content = ""
    ...
    blocked = any(kw in response_content.lower() for kw in refuse_keywords)
    return {
        "allowed":        not blocked,
...
async def check_output_rail(question: str, answer: str, rails=None) -> dict:
    ...
    refuse_keywords = ["xin lỗi", "không thể cung cấp", "i cannot", "tôi không thể"]
    flagged = (response_content != answer) or any(kw in response_content.lower() for kw in refuse_keywords)
    return {
        "safe":           not flagged,
        "flagged_reason": "nemo_output_rail" if flagged else None,
        "final_answer":   response_content if flagged else answer,
    }
...
def save_phase_c_report(results: list[dict], latency: dict, path: str = "reports/guard_results.json"):
